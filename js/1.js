$(function() {
	$('#container').highcharts({
		...
		xAxis: {
			type: 'datetime', 
			// maxZoom: 14 * 24 * 3600000, // fourteen days title: { text: null }
			labels: {
				// formatter: function() {
		...
		yAxis: {
			...
			labels: {
				formatter: function() {
					return this.value / 1000 + '4444444444444';
				}
			}
		},
		tooltip: {
			pointFormat: '{series.name} produced <b>{point.y:,.0f}</b><br/>warheads in {point.x} 55555555555555555'
		},
		plotOptions: {
			area: {
				pointStart:  Date.UTC(2015,11, 12),
